...
def get_info():
    return jsonify(
        descricao = "gerenciamento de comentarios do Pontos Turísticos - Orlando",
        versao = "1.0"
    )
...
def get_comentarios(id_do_feed, pagina, tamanho_pagina):
    ...
    cursor.execute("SELECT id as comentario_id, feed as lugar_id, comentario, nome, conta, DATE_FORMAT(data, '%Y-%m-%d %H:%i') as data " +
                   "FROM comentarios " +
